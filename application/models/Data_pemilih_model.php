...
class Data_pemilih_model extends CI_Model
{
    ...
    function getKelas_by_idkelas($id)
    {
        ...
        if ($q->num_rows() > 0) {
            // log_message('info', 'ID Kelas: ' . $row->idkelas);
            return $q->row(); // Mengembalikan satu baris data
        } else {
            return false;
            log_message('info', 'ID Kelas: ' . $row->idkelas); // Mengembalikan false jika tidak ada data
        }
    }
